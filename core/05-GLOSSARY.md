...
| **Actor–role capability match** | The principle that each lifecycle role is filled by an actor whose capability fits the role's **leverage and failure-cost** — strongest on design and verification/review, mid-tier where work is well-specified and mistakes are cheap and caught downstream, the orchestrator sized for coordination range. Vendor-neutral (roles and tiers, not named models or people); concrete bindings live in the adapter/project layer. See [`00-PHILOSOPHY.md`](00-PHILOSOPHY.md) Principle 12. |
...
| **Bounded delegation depth** | The **hard cap of five levels** on how deep delegated checking/verifying may nest: the agent running the gate is level 1, each fresh reviewer/subagent it spawns is the next level, and level 5 is the ceiling (it may not spawn a level-6 checker). A ceiling that bounds the pathological verifier-of-a-verifier stack — **not** a target; the practice stays flat single passes with escalation to the owner past a bounded pass (Principle 11). Distinct from **breadth** (K parallel reviewers within one round, all at the same level — unaffected by the cap, which counts only *nesting*). See [`03-REVIEW-GATES.md`](03-REVIEW-GATES.md) §6 and [`00-PHILOSOPHY.md`](00-PHILOSOPHY.md) Principle 11. |
