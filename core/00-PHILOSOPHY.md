...
| 11 | **Challenge — adversarially, and bound it** | An agent that only agrees is a worse collaborator than one that pushes back; but unbounded self-doubt never ships. The value is in the *bounded* adversarial stance. | Challenge the **owner** — state a real opinion, push back, never obey blindly; the owner's input and the agent's judgment are both first-class. Challenge **yourself** — when you push back or propose a rule, run **one** adversarial pass that attacks your own position (a fresh reviewer critiquing your draft), then **decide**; bound it to that single pass so rigor never becomes paralysis. Past one pass, escalate to the **owner**, not another reviewer round. This is self-review (#6) turned adversarial and bounded; the housekeeping case of it is Ritual 15. The same bound has a **hard ceiling in depth**: delegated checking/verifying nests **at most five levels** — a ceiling that bounds the pathological verifier-of-a-verifier stack, never a target to reach ([`03-REVIEW-GATES.md`](03-REVIEW-GATES.md) §6). At the ceiling, escalate to the owner, never descend another level. |
| 12 | **Match the actor to the role's leverage** | The lifecycle's roles carry unequal stakes: a flawed design or a defect missed at review costs far more than a slow implementation. Staffing every role with the same actor either overpays on the cheap roles or underpowers the expensive ones. | Fill each role with an actor whose capability fits the role's **leverage and failure-cost**: give the highest-stakes roles — **design** and **verification/review** — your strongest available actor; a mid-tier actor is fine where the work is well-specified and a mistake is cheap and caught downstream (much **implementation**, routine **planning**), and the **orchestrator** needs coordination range more than raw depth. This is stated in *roles and capability tiers* and is vendor-neutral — the **concrete bindings** (which specific model, skill level, or person fills each role) live in the **adapter/project layer**, never in `core/`. It reads the same for a human team (a senior architect designs and reviews; a mid-level engineer implements) and for AI actors. Never staff a role *below* the floor its failure-cost demands merely to save cost. |
...
- **#5 + #6 + #11** are the collaboration stance: the agent *recommends* (#5), is its own *first reviewer* (#6), and actively *challenges* — the owner and itself — yet **bounds** the self-challenge so rigor never becomes paralysis (#11). The agent owns the *review gate*; the owner owns the *decision gate*. The agent never asks the owner to do the review it should have done; the owner is never bypassed on a binding decision.
